...
def compile_data_designer_config(
    ...
    _resolve_and_add_seed_columns(config, resource_provider.seed_dataset_reader)
    _validate(config)

    return config


def _resolve_and_add_seed_columns(config: DataDesignerConfig, seed_reader: SeedDatasetReader | None) -> None:
