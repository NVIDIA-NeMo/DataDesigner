...
def test_from_config_loads_yaml_url(mock_urlopen: MagicMock, stub_data_designer_config_str: str) -> None:
    mock_response = MagicMock()
    mock_response.read.return_value = stub_data_designer_config_str.encode("utf-8")
...
@patch("data_designer.config.utils.io_helpers.urlopen")
def test_from_config_loads_json_url(mock_urlopen: MagicMock, stub_data_designer_config_str: str) -> None:
    config_dict = yaml.safe_load(stub_data_designer_config_str)
    mock_response = MagicMock()
    mock_response.read.return_value = json.dumps(config_dict).encode("utf-8")
    mock_urlopen.return_value.__enter__.return_value = mock_response
