...
def normalize_claude_directory(root_path: Path, matched_files: list[Path]) -> list[NormalizedTraceRecord]:
    ...
    for file_path in trace_files:
        try:
            ...
            logger.warning("Skipping malformed Claude Code trace file %s: %s", file_path, error)

    if not normalized_records:
        raise TraceSeedParseError(f"No valid Claude Code session JSONL files found under {root_path}")
        ...
        try:
            ...
            for _, raw_record in rows:
                ...
                if record_type == "event_msg":
                    ...
                    if event_type == "agent_reasoning" and (
                        reasoning_text := _coerce_optional_str(payload.get("text"))
                    ):
                    ...
                    messages.append(
                        _build_message(
                            ...
                            tool_call_id=_require_string(
                                payload.get("call_id"), f"Codex tool output id in {file_path}"
                            ),
                        )
                    )
            ...
            logger.warning("Skipping malformed Codex rollout file %s: %s", file_path, error)

    if not normalized_records:
        raise TraceSeedParseError(f"No valid Codex rollout JSONL files found under {root_path}")
    return normalized_records
...
def normalize_chat_completion_directory(root_path: Path, matched_files: list[Path]) -> list[NormalizedTraceRecord]:
    ...
    for file_path in trace_files:
        try:
            rows = _load_jsonl_rows(file_path)
            if not rows:
                logger.warning("Skipping empty chat-completion JSONL file %s", file_path)
                continue
            ...
            for line_number, raw_record in rows:
                ...
                source_meta["source_file_line"] = line_number
            ...
            logger.warning("Skipping malformed chat-completion JSONL file %s: %s", file_path, error)

    if not normalized_records:
        raise TraceSeedParseError(f"No valid chat-completion JSONL files found under {root_path}")
    return normalized_records
...
def _load_claude_session_index(root_path: Path) -> dict[str, dict[str, Any]]:
    ...
    for index_path in sorted(root_path.rglob("sessions-index.json")):
        ...
        except (TraceSeedParseError, json.JSONDecodeError) as error:
