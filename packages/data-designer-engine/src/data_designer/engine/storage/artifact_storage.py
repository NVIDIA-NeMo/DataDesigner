...
class ArtifactStorage(BaseModel):
    ...
    def load_processor_dataset(self, processor_name: str) -> pd.DataFrame:
        """Load the dataset generated by a specific processor.

        Handles both storage layouts: a directory of batch parquet files
        (from batched runs) and a single parquet file (from preview runs).

        Args:
            processor_name: Name of the processor whose output to load.

        Returns:
            DataFrame containing the processor's output dataset.

        Raises:
            ArtifactStorageError: If no artifacts exist for the given processor name.
        """
        dir_path = self.processors_outputs_path / processor_name
        file_path = self.processors_outputs_path / f"{processor_name}.parquet"
        if dir_path.is_dir():
            return read_parquet_dataset(dir_path)
        if file_path.is_file():
            return read_parquet_dataset(file_path)
        raise ArtifactStorageError(f"No artifacts found for processor named {processor_name!r}")

    def list_processor_names(self) -> list[str]:
        """Discover processor names from the processor outputs directory.

        Finds both directory-based outputs (from batched runs) and
        single-file outputs (from preview runs).

        Returns:
            List of processor names that have written output artifacts.
        """
