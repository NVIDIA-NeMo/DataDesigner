...
class AgentContextService:
    _family_specs: dict[str, FamilySpec] = {
        ...
        "constraints": FamilySpec(
            name="constraints", type_union=ColumnConstraintT, discriminator_field="constraint_type"
        ),
    }

    def get_family_names(self) -> list[str]:
        return sorted(self._family_specs)

    def get_family_spec(self, family: str) -> FamilySpec:
        spec = self._family_specs.get(self._normalize_family_name(family))
        if spec is None:
            raise AgentContextError(
                code="unknown_family",
                message=f"Unknown family {family!r}.",
                details={"available_families": self.get_family_names()},
            )
        return spec

    def discover_family_types(self, family: str) -> dict[str, type]:
        spec = self.get_family_spec(family)
        discovered: dict[str, type] = {}
        for model in get_args(spec.type_union):
            type_name = self._extract_literal_value(model.model_fields[spec.discriminator_field].annotation)
    ...
    def get_import_path(self, cls: type) -> str:
        exported = getattr(dd, cls.__name__, None)
        if exported is cls:
            return f"data_designer.config.{cls.__name__}"
        return f"{cls.__module__}.{cls.__qualname__}"

    def get_family_catalog(self, family: str) -> list[dict[str, str]]:
        return [
            {"type_name": type_name, "class_name": cls.__name__, "import_path": self.get_import_path(cls)}
            for type_name, cls in self.discover_family_types(family).items()
        ]

    def get_family_schema(self, family: str, type_name: str) -> dict[str, Any]:
        spec = self.get_family_spec(family)
        types_map = self.discover_family_types(family)
        ...
        return self._build_schema_dict(spec, type_name, cls)

    def get_family_schemas(self, family: str) -> dict[str, Any]:
        spec = self.get_family_spec(family)
        types_map = self.discover_family_types(family)
        items = [self._build_schema_dict(spec, type_name, cls) for type_name, cls in types_map.items()]
        return {"family": spec.name, "items": items}

    def get_builder_api(self) -> dict[str, Any]:
        return {
            "class_name": DataDesignerConfigBuilder.__name__,
            "import_path": self.get_import_path(DataDesignerConfigBuilder),
            "methods": self._get_builder_methods(),
        }
    ...
    def get_context(self, config_dir: Path) -> dict[str, Any]:
        catalogs = {family: self.get_family_catalog(family) for family in self.get_family_names()}
        return {
            "operations": self.get_operations(),
            "families": [{"family": family, "count": len(items)} for family, items in catalogs.items()],
            "types": catalogs,
            "state": {
                "model_aliases": self.get_model_aliases_state(config_dir),
                "persona_datasets": self.get_persona_datasets_state(config_dir),
            },
            "builder": self.get_builder_api(),
        }

    def get_types(self, family: str | None) -> dict[str, Any]:
        if family is None:
            catalogs = {name: self.get_family_catalog(name) for name in self.get_family_names()}
            return {
                "families": [{"family": name, "count": len(items)} for name, items in catalogs.items()],
                "items": catalogs,
            }
        return {"family": self.get_family_spec(family).name, "items": self.get_family_catalog(family)}
    ...
    def get_schema(self, family: str, type_name: str | None, *, all_types: bool) -> dict[str, Any]:
        ...
        if all_types:
            return self.get_family_schemas(family)
        ...
        return self.get_family_schema(family, type_name)

    def get_model_aliases_state(self, config_dir: Path) -> dict[str, Any]:
        model_registry = self._load_registry(ModelRepository(config_dir))
        provider_registry = self._load_registry(ProviderRepository(config_dir))
            ...
            items.append(
                {
                    ...
                    "generation_type": getattr(
                        model_config.generation_type, "value", str(model_config.generation_type)
                    ),
    ...
    def get_persona_datasets_state(self, config_dir: Path) -> dict[str, Any]:
    ...
    def _build_schema_dict(self, spec: FamilySpec, type_name: str, cls: type) -> dict[str, Any]:
        return {
            "family": spec.name,
            "type_name": type_name,
            "class_name": cls.__name__,
            "import_path": self.get_import_path(cls),
    ...
    def _normalize_family_name(self, family: str) -> str:
        ...
        if normalized in self._family_specs:
            return normalized
        plural = f"{normalized}s"
        if plural in self._family_specs:
            return plural
        return normalized

    def _extract_literal_value(self, annotation: Any) -> str:
    ...
    def _get_builder_methods(self) -> list[dict[str, Any]]:
        ...
        for name, attr in inspect.getmembers(DataDesignerConfigBuilder):
            ...
            methods.append(
                {
                    "name": name,
                    "signature": self._format_signature(name, signature),
                    "summary": self._get_first_line(docstring),
                    "docstring": docstring,
                }
            )
    ...
    def _format_signature(self, method_name: str, signature: inspect.Signature) -> str:
