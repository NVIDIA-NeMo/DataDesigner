...
and `codex`; `atif` expects standalone `.json` trajectory files and requires `--trace-dir`, while `claude_code`,
`codex`, and `hermes_agent` can use their default locations when `--trace-dir` is omitted. The pipeline turns each
