...
- **Column generator plugins**: Custom [column generators](../code_reference/generators.md) you pass to the config builder's [add_column](../code_reference/config_builder.md#data_designer.config.config_builder.DataDesignerConfigBuilder.add_column) method.
- **Seed reader plugins**: Custom [seed readers](../code_reference/seed_readers.md) that load data from new sources, such as databases, cloud storage, or custom file formats.
- **Processor plugins**: Custom [processors](../code_reference/processors.md) that transform data before batches, after batches, or after generation completes. Pass them to the config builder's [add_processor](../code_reference/config_builder.md#data_designer.config.config_builder.DataDesignerConfigBuilder.add_processor) method.
