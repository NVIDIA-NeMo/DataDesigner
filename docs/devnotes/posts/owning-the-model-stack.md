...
![Owning the Model Stack: from opaque dependency to transparent, controllable client layer](assets/native-model-client-architecture/native-model-client-hero.png)
...
The biggest opportunity was **adaptive concurrency**. When a provider starts returning 429s at scale, you don't want to just retry — you want the system to *learn* the provider's actual capacity and adjust on the fly. That adaptation needs to be aware of your pipeline's topology: which models share an endpoint, which routes share a rate-limit budget, how sync and async callers interact. Building that required owning the transport layer.
...
The replacement is a layered stack where each layer does one thing. Importantly, `ModelFacade` — the public orchestration surface that column generators call — didn't change at all. Everything below it is new.

![Native model client architecture: five layers from ModelFacade down to provider HTTP APIs](assets/native-model-client-architecture/native-model-client-layers.png){ style="max-width:100%; height:auto" }
...
With the native client layer in place, we had the foundation to build something that wasn't possible before: adaptive concurrency control. Let's start with the problem.
...
The asymmetry is deliberate. You probe upward slowly because overshooting wastes requests. You pull back quickly because staying above the limit wastes *everything* — every request in the burst gets rejected. This is the same insight that makes TCP work: be optimistic cautiously, be pessimistic decisively.
...
![AIMD concurrency control over time: initial phase, 429 drop, recovery, ceiling stabilization, steady state](assets/native-model-client-architecture/aimd-concurrency-over-time.png){ style="max-width:100%; height:auto" }
...
Here's a subtlety that bit us during testing. When the system is running at capacity and the provider starts returning 429s, it's not just one request that fails — multiple in-flight requests hit the rate limit at the same time. Without dampening, each 429 triggers its own multiplicative decrease. If you have 5 concurrent 429s and each one cuts the limit by 25%, you've collapsed from 20 to 4 in a single burst. That's way too aggressive.
...
![AIMD per-request lifecycle: acquire, call, branch on success/429/error, release](assets/native-model-client-architecture/aimd-lifecycle.png){ style="max-width:100%; height:auto" }
...
![Two-level throttle keying: global cap per provider+model, independent domain states for chat, embedding, image](assets/native-model-client-architecture/throttle-keying.png){ style="max-width:100%; height:auto" }
...
The practical effect: a burst of 429s on the chat route doesn't starve embedding requests, and vice versa. Each route adapts to its own capacity independently while respecting the shared upstream limit.
...
The transport layer (via `httpx` with `RetryTransport`) handles transient server failures — 502, 503, 504, and connection errors. These are hiccups. The server is temporarily broken. Retry with exponential backoff and jitter, and move on.

But **429 is explicitly excluded from transport retries**.

![Retry boundary: 502/503/504 retried at transport, 429 passed through to ThrottledModelClient for AIMD feedback](assets/native-model-client-architecture/retry-boundary.png){ style="max-width:100%; height:auto" }
