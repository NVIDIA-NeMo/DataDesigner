...
- Generate structured responses whose evaluator marks all required attributes present.
...
| Hugging Face Hub | Record-selection-specific publishing is deferred to a generic publishing workstream |
...
- Image-generation columns are rejected with an actionable error that names the unsupported columns.
...
rows. It preserves the published output schema when every candidate batch accepts zero rows; it is not included in
accepted counts. The marker's `schema_materialized` flag records whether its candidate produced a typed output schema.
Resume discards an anchor not owned by committed work, reconstructs one from an accepted partition when possible,
and rejects a missing committed typed schema rather than silently changing it.
...
Resume replays an exhausted early shutdown; when candidate budget remains, it continues from the next committed offset.
...
`selection-accepted/` partitions remain the source of truth. Resume discards incomplete publication staging and
rebuilds it without regenerating candidates.
...
3. Write the post-trim rows to a deterministic immutable accepted-partition path when non-empty.
4. Write accepted-only dropped-column and post-batch processor artifacts under the same candidate batch ID.
...
- Verify every marker that names an accepted partition points to a readable file whose row count equals
  `accepted_records`.
- Delete uncommitted accepted partitions for the next candidate batch before rerunning that batch.
...
- When `post_generation_state` is `started` or missing for a terminal marker set, discard incomplete publication
  staging, rebuild clean staging, and replace `parquet-files/` at promotion time. Never delete immutable accepted
  partitions or marker-committed post-batch processor artifacts during publication recovery.
...
output, and processor outputs before fresh generation starts; it must never silently combine incompatible runs.
...
Image-generation columns are rejected at the build boundary in V1 because filtering a parquet row cannot safely
clean up media already written by a rejected candidate. The error names every unsupported image column and directs
the caller to remove it or disable record selection. Text side-effect columns stored in the row are filtered with
the DataFrame. Custom and plugin predicates must remain side-effect-free because record selection does not track
external files they create.

## Hugging Face Hub publication

Record-selection-specific Hub publication is outside this V1 implementation. Atomic commits, pinned filesystem
snapshots, TOCTOU protection, and stale-file cleanup apply to ordinary datasets too, so they belong in a separate
repository-wide publishing workstream rather than a selection-only upload path.
...
### Phase 4: Durable selection checkpoints, publication, and resume
...
- `interface/results.py` — represent schema-bearing empty partial results without synthetic profiling output.
...
- Explicit after-generation processor, image-column, and external-side-effect limitations.
- Hub publication improvements remain in a separate generic publishing workstream.
...
- Image-generation columns are rejected before generation and the error names all unsupported columns.
...
| Rejected media leaks disk space | Reject image-generation columns before candidate generation in V1 |
...
| Generic Hub publication needs atomic replacement | Address it once in a separate repository-wide publishing workstream |
...
- Reject image-generation columns before generation in v1.
...
- Reuse `post_generation_state` so selection completion and publication completion are distinct; only `complete` is
  exposed as locally complete, and interrupted mutable output is rebuilt from immutable accepted partitions.
- Replace stale local published data during recovery and clear engine-managed local prefixes on incompatible
  `IF_POSSIBLE` restart.
- Defer record-selection-specific Hugging Face behavior to a generic publishing workstream.
...
- Clear V1 rejection of image-generation columns with actionable column names.
