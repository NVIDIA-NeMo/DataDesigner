...
Status: implemented on the `codex/790-cohort-retry-workflow` comparison branch in draft [PR #830](https://github.com/NVIDIA-NeMo/DataDesigner/pull/830)
...
| `cohort_retry_builders.py` | Deep-snapshot the original builder and project isolated base, attempt, and final builders. It also reports when seed or preserved-sampler values actually require a base materialization run. Hidden runs force generated columns to remain materialized and omit user processors/profilers. |
| `cohort_retry_state.py` | Define the public Pydantic wire models for retry, attempt, and terminal completion state; derive canonical attempt paths from list position; validate/restart resume state; and provide atomic JSON, parquet, and file writes. |
| `cohort_retry_utils.py` | Provide public stateless operations for classification, stable-value validation, accepted-partition coalescing, media handling, usage aggregation, summary derivation, and terminal-artifact cleanup. These operations can be unit-tested without constructing a runner. |
| `cohort_retry_runner.py` | Orchestrate the bounded state transition loop and ordinary internal Data Designer runs. It wires builders, durable state, and public utility functions together rather than owning their validation and transformation logic. |
...
- **Seedless runs have slot identity but no source population.** When no seed or preserved sampler requires materialization, the runner constructs an in-memory `N`-row frame and adds stable slot IDs directly. This supplies logical retry identity, not a user-visible source distribution, and avoids an extra base generation run. The current builder projection still requires the original builder to contain model configurations for the generated attempt columns.
...
- Stateless classification, coalescing, stable-value validation, media, usage, and summary functions are tested directly without constructing `CohortRetryRunner`.
...
The loop is implemented as the workflow-level `CohortRetryRunner`, not inside `DatasetBuilder` or `AsyncTaskScheduler`. `CompositeWorkflow` recognizes the declarative stage policy and delegates the retry stage to that collaborator. The runner is intentionally orchestration-only: durable schemas, deterministic paths, and atomic writes live in `cohort_retry_state.py`, while stateless classification, validation, coalescing, media, metadata, and usage operations live as public functions in `cohort_retry_utils.py`. This keeps the lifecycle visible while allowing those operations to be tested independently of runner state.
...
`AttemptManifest` persists only outcome counts and durable usage for one committed attempt. Its list position determines the attempt index and therefore all attempt paths. `RetryManifest` persists the retry facts needed for validation and recovery, while constant storage locations and distribution-warning text are computed at the boundary that exposes them. Tests target the state models and public utility functions directly, reserving runner tests for orchestration, resume, and commit-order behavior.
