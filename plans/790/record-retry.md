# Record Retry Plan (Cohort-Preserving Workflow Design)

Status: implemented on the historical `codex/790-cohort-retry-workflow` comparison branch in draft [PR #830](https://github.com/NVIDIA-NeMo/DataDesigner/pull/830)
...
| Complexity from earlier designs | Cohort-preserving record-retry response |
|---|---|
...
- A mode-aware `RecordRetryBuilderFactory` that removes sampler configs in preserve mode and retains them in resample mode.
...
`RetryUntil` is a frozen Pydantic model rather than a mutable dataclass or a hand-serialized policy object. Pydantic validates its typed fields, forbids unknown fields, accepts enum values through the normal model boundary, and provides the JSON-mode representation used by fingerprints and persisted state. `max_attempts` and `max_candidate_records` are individually optional, but at least one must be present. Both are strict positive integers: Boolean, float, and string values are rejected rather than coerced. `max_attempts` includes the initial full-cohort generation as attempt 1; it is the total number of times a still-pending cohort may run, not the number of retries after the first run. At runtime, `max_candidate_records` must be at least the stage's `num_records`, because the implementation never starts a partial cohort attempt.
...
|---|---|
| `record_retry.py` | Define the public enums and frozen, typed Pydantic `RetryUntil` policy. |
| `record_retry_builders.py` | Provide `RecordRetryBuilderFactory`, which deep-snapshots the original builder and creates isolated base, attempt, and final builders. It also reports when seed or preserved-sampler values require a base materialization run. Hidden runs force generated columns to remain materialized and omit user processors/profilers. |
| `record_retry_attempts.py` | Provide `RecordRetryAttemptRunner`, which owns base materialization, immutable attempt execution or recovery, completion markers, strict classification, accepted-partition commits, seed-media snapshots, accepted-media packaging, and the task traces produced by those phases. |
| `record_retry_publisher.py` | Provide `RecordRetryPublisher`, which coalesces committed accepted partitions, publishes the nonempty terminal create or explicit empty result, validates final completion, restores the original builder identity, and writes canonical metadata and aggregate usage. |
| `record_retry_state.py` | Define the strict Pydantic wire models for retry, attempt, and terminal completion state. `RetryManifest.policy` is a typed `RetryUntil`, not an unvalidated dictionary. The module also derives canonical attempt paths from list position, validates/restarts resume state, and provides atomic JSON, parquet, and file writes. |
| `record_retry_utils.py` | Provide public stateless operations for classification, stable-value validation, accepted-partition coalescing, media handling, usage aggregation, summary derivation, and terminal-artifact cleanup. These operations can be unit-tested without constructing a runner. |
| `record_retry_runner.py` | Provide a thin `RecordRetryRunner` that validates the request, composes the builder factory, attempt runner, and publisher, owns the bounded pending-slot loop and exhaustion policy, and is the sole writer of the root retry manifest. |
...
```mermaid
flowchart LR
    CW["CompositeWorkflow"] --> CR["RecordRetryRunner<br/>bounds + root manifest"]
    CR --> BF["RecordRetryBuilderFactory<br/>phase-specific builders"]
    CR --> AR["RecordRetryAttemptRunner<br/>base + immutable attempts"]
    CR --> PB["RecordRetryPublisher<br/>coalesce + terminal publication"]
    AR --> BF
    PB --> BF
    CR --> ST["typed state + public utilities"]
    AR --> ST
    PB --> ST
```

The four runtime collaborators have deliberately non-overlapping ownership. The thin runner does not call `DataDesigner._create()` or construct phase builders. It is the only collaborator that writes `manifest.json`; the attempt runner and publisher write their phase-specific completion artifacts and return durable facts to the runner for the root commit. The attempt runner owns only base and attempt execution, while the publisher owns only terminal publication. The builder factory is a pure configuration collaborator with no filesystem or execution responsibility. Persisted Pydantic models and public utility functions remain data/operation boundaries rather than wrapper classes.

`RecordRetryRunner` returns the ordinary stage-root `DatasetCreationResults`, not a candidate result plus a separate selected path. The canonical result therefore continues to work with normal workflow loading, export, processor-output selection, and Hub publication. The root `metadata.json` contains a `record_retry` summary; `workflow-metadata.json` contains the policy and the stage's `retry_summary`. `RecordRetryExhaustedError`, a `DataDesignerWorkflowError` subclass, exposes target, accepted, unresolved, candidate, attempt, and unresolved-slot fields when `on_exhausted="raise"` reaches a bound.
...
- **Record retry with preserved samplers** fixes the initial seed and sampler values, then retries downstream generation for each rejected logical row. When all rows eventually pass, the output preserves the initial realized cohort distribution exactly.
- **Record retry with resampled samplers** fixes seed values but redraws sampler values for rejected slots. This can improve completion, but accepted sampler values become conditioned on predicate success.
...
4. Use that pending-slot slice as the next attempt's seed dataset.
...
If the original builder has neither a seed dataset nor sampler columns that must be materialized in `preserve` mode, no base generation run is needed. `RecordRetryAttemptRunner` creates a direct in-memory `N`-row frame, assigns the stable slot column, and persists it as the base cohort. This avoids a synthetic user column and an otherwise empty ordinary generation run; sampler columns in `resample` mode are still generated by each attempt.
...
│   ├── images/              # durable snapshot of referenced LocalFile seed media, when present
│   └── run/                 # only when an ordinary base-materialization run is required
├── attempts/
│   ├── attempt-000/
│   │   ├── input.parquet
...
The root `RetryManifest` records the complete retry policy, stable internal column names, target count, base seed-column names, committed `AttemptManifest` accounting, per-phase model usage, status, and unresolved slot IDs. It intentionally stores only durable facts. An attempt's index is its stable position in the manifest list, and its input, run, and accepted-partition paths are derived from that index. Whether samplers executed is derived from `sampler_retry_mode`; it is not a redundant per-attempt flag. The constant base-cohort path and a partial result's distribution warning are likewise derived rather than persisted. Unlike the completion marker, appending the root manifest entry commits classification and advances pending-slot and candidate accounting.
...
`RecordRetryBuilderFactory` supplies each phase-specific builder. `RecordRetryAttemptRunner` performs the base work and one immutable attempt at a time. The thin `RecordRetryRunner` owns the `pending` loop, bounds, exhaustion decision, and root attempt-manifest commit. After the loop, `RecordRetryPublisher` owns coalescing and terminal publication.

```text
if seed or preserved-sampler values require materialization:
    materialize seed values and, in preserve mode, sampler values through an ordinary unprofiled internal create
    require exactly N rows
    copy referenced LocalFile seed media once into base-cohort/images/
else:
    construct a direct in-memory N-row frame
...
When a base run is required, it and all attempt and nonempty final runs use the ordinary private `_create()` lifecycle. Base and attempt runs disable profiling; attempts additionally allow a successfully completed zero-row internal result. A seedless cohort with no preserved samplers skips the base `_create()` call and starts from the direct stable-slot frame. `allow_empty=True` does not turn generation exceptions into successful empty attempts: builder failures still fail normally. Since every attempt contains at most one candidate for each pending slot, accepted rows cannot overshoot `N`, and final trimming is unnecessary.
...
- `raise`: write an `exhausted` manifest, retain all durable attempts, and raise `RecordRetryExhaustedError` with structured target, accepted, unresolved, candidate, attempt, and unresolved-slot fields.
...
- A stage's ordinary workflow `output_processors`, when configured, still run after the record-retry runner returns and only for a nonempty result. They are outside the publisher's exact row-count assertion.
- Every base, attempt, nonempty final, and workflow output-processor `_create()` invocation owns an ordinary OpenTelemetry create lifecycle. This preserves normal per-phase request and scheduler telemetry, but v1 does not add one enclosing create span that represents the entire logical record-retry stage.

The publisher can aggregate only usage that reached a durable snapshot. Attempt usage becomes durable in `attempt-completion.json`; if a process stops after provider calls but before that marker, a resumed ordinary create may no-op from disk without reconstructing the previous process's in-memory usage delta. Final-phase usage becomes durable in `final-completion.json`, and the logical aggregate becomes reusable through canonical root metadata. Task traces describe the current invocation; traces from an interrupted process are not persisted by either marker.
...
- **Cohort orchestration is in-memory.** The attempt runner loads or creates the complete base cohort as a pandas DataFrame and filters that frame for each attempt; the publisher reads and concatenates all accepted partitions before terminal publication. Peak memory is therefore O(`N`) with additional full-frame copies, rather than the bounded batch memory of the normal engine path. Large cohort targets can exhaust process memory even when an ordinary `create()` of the same size would remain within its configured buffer.
- **Validation and packaging scale with candidate cells.** Every nonempty attempt verifies stable seed/cohort values row-by-row and column-by-column, and accepted media packaging recursively scans every accepted cell. Selecting the next pending-slot input also scans the in-memory base cohort each time. These checks favor explicit invariants over throughput and can add substantial Python and filesystem overhead for wide cohorts, deeply nested values, many attempts, or media-heavy rows.
...
- **All-zero partials use a direct publication path.** The publisher can emit a correct schema and configured drop projection, but it cannot infer arbitrary schema-transforming behavior from terminal processors without executing them. Analysis is absent, public `load_analysis()` raises `DataDesignerProfilingError`, and original terminal processors/profiling are skipped.
...
- **Media packaging is local and image-path-specific.** Preserved relative `images/...` seed references are snapshotted during base materialization only when their root can be resolved from a `LocalFileSeedSource` with an adjacent ancestor `images/` directory, then copied from that snapshot into each attempt. Relative media from an in-memory, remote, or otherwise unresolvable seed source fails explicitly; absolute paths and generic plugin side effects are not imported. The attempt runner then rewrites files that resolve inside an attempt artifact and copies accepted files below root `images/attempt-NNN/`. It does not define a generic media/side-effect transaction for plugins or external systems, and rejected media remains in retained attempt artifacts.
- **Seedless runs have slot identity but no source population.** When no seed or preserved sampler requires materialization, the attempt runner constructs an in-memory `N`-row frame and adds stable slot IDs directly. This supplies logical retry identity, not a user-visible source distribution, and avoids an extra base generation run. `RecordRetryBuilderFactory` still requires the original builder to contain model configurations for the generated attempt columns.
...
| Earlier experiment | Record retry (cohort-preserving) |
|---|---|
...
- A seedless builder receives stable internal slots from the direct `N`-row base frame without running a synthetic generator or exposing an implementation-only column.
- Manifest wire models reject redundant derived attempt indices, paths, sampler-execution flags, base paths, and distribution-warning text; public helpers reconstruct the expected paths and summaries.
- Stateless classification, coalescing, stable-value validation, media, usage, and summary functions are tested directly without constructing `RecordRetryRunner`.
...
The loop is implemented at the workflow boundary, not inside `DatasetBuilder` or `AsyncTaskScheduler`. `CompositeWorkflow` recognizes the declarative stage policy and delegates the retry stage to a thin `RecordRetryRunner`. That runner validates the requested bounds, constructs the three phase collaborators, loads compatible durable state, owns the pending-slot loop and exhaustion policy, commits each returned attempt summary, invokes terminal publication, and finally marks the root manifest complete. It does not construct builders, call `DataDesigner._create()`, classify candidate rows, package media, coalesce partitions, or publish canonical results.

The runtime collaboration is divided by lifecycle ownership:

- `RecordRetryBuilderFactory` owns the deep original-config snapshot and constructs the base, attempt, and final builders. It has no filesystem or execution responsibility.
- `RecordRetryAttemptRunner` owns base materialization and one immutable attempt transaction. It is the only collaborator that accumulates base/attempt task traces, writes attempt completion markers, classifies output, packages accepted media, and returns `AttemptManifest` accounting to the thin runner for the root commit.
- `RecordRetryPublisher` owns accepted-partition coalescing and terminal publication. It runs or recovers the final create, checks the row-count invariant, writes the final completion marker, publishes the explicit empty case, aggregates durable usage, restores the original builder identity, and returns the canonical `DatasetCreationResults` plus the final phase usage needed for the root commit.
- `RecordRetryRunner` owns control flow and root state only: pending slots, bounds, exhaustion, all `manifest.json` transitions, and collaboration among the other three objects. Its constructor accepts attempt-runner and publisher collaborators, with production defaults, so orchestration tests can inject local instances rather than patch process-wide class behavior.

Durable schemas, deterministic paths, resume loading, and atomic writes live in `record_retry_state.py`. Stateless classification, validation, coalescing, media, metadata, and usage operations live as public functions in `record_retry_utils.py`. These functions are implementation surfaces that can be tested directly; they are not hidden behind forwarding-only utility classes.

End-to-end responsibilities across those collaborators:

- materialize the immutable seed cohort and, in `preserve` mode, its sampler values;
- construct the direct stable-slot frame instead when no base values need materialization;
...
`AttemptManifest`, `AttemptCompletion`, `FinalCompletion`, and `RetryManifest` are strict Pydantic wire models rather than ad hoc dictionaries. `RetryManifest.policy` embeds the frozen, typed `RetryUntil` model, so resume validation compares a validated policy rather than independently parsed keys. `AttemptManifest` persists only outcome counts and durable usage for one committed attempt. Its list position determines the attempt index and therefore all attempt paths. `RetryManifest` persists the retry facts needed for validation and recovery, while constant storage locations and distribution-warning text are computed at the boundary that exposes them. Tests target the state models and public utility functions directly, reserve thin-runner tests for orchestration and manifest sequencing, and exercise attempt execution and publication through their own public collaborators.
...
2. Materialize seed columns once as a fixed base cohort when present; otherwise create the fixed cohort as a direct `N`-row stable-slot frame.
3. Retry only rejected slots by selecting them from the immutable original cohort.
