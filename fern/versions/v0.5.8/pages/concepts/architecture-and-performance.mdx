...
Data Designer stores checkpoint state in `metadata.json` and the run configuration in `builder_config.json`. The sync engine resumes from `num_completed_batches`; the async engine scans completed `batch_*.parquet` row groups and reads parquet metadata to recover the actual row count already persisted.
...
- Only one writer should target a dataset directory at a time.
