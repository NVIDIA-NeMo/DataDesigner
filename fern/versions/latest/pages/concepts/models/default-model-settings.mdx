...
- **Models**: Access to a wide variety of models through [OrcaRouter](https://www.orcarouter.ai)'s unified API
- **Getting Started**: Get your API key from [orcarouter.ai](https://www.orcarouter.ai)

The OrcaRouter provider gives you access to a unified interface for many different language models from various providers through an OpenAI-compatible gateway.
...
| Alias | Model | Use Case | Inference Parameters |
|-------|-------|----------|---------------------|
| `orcarouter-text` | `openai/gpt-5.5` | General text generation | `temperature=0.85, top_p=0.95` |
| `orcarouter-reasoning` | `openai/gpt-5.5` | Reasoning and analysis tasks | `extra_body={"reasoning_effort": "medium"}` |
| `orcarouter-vision` | `openai/gpt-5.5` | Vision and image understanding | `extra_body={"reasoning_effort": "medium"}` |
...
<Warning title="Hosted Provider Data Handling">
  The default model providers call hosted endpoints operated by NVIDIA, OpenAI, OpenRouter, OrcaRouter, or their upstream providers. Provider terms and privacy practices apply independently of Data Designer, and free or trial endpoints may log request data for security, operations, or product improvement. Do not submit confidential information or personal data, including faces, voices, screenshots, regulated data, or other sensitive content, unless the selected provider and endpoint are approved for your use case.
</Warning>
...
- **[Configure Model Settings With the CLI](/concepts/models/configure-with-the-cli)**: Learn how to use the CLI to manage model settings
- **[Model Configurations](/concepts/models/model-configs)**: Learn about model configurations
