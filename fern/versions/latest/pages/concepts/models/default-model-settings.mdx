...
|-------|-------|----------|---------------------|
| `nvidia-text` | `nvidia/nemotron-3.5-lightning-30b-a3b` | General text generation | `temperature=1.0, top_p=0.95, extra_body={"chat_template_kwargs": {"enable_thinking": False}}` |
...
<Note title="Existing configurations are preserved">
  Upgrades do not overwrite existing configuration files. If `nvidia-text` still references a retired model, update `~/.data-designer/model_configs.yaml` manually or run `data-designer config reset` to restore the current defaults. Resetting deletes all custom configurations.
</Note>
