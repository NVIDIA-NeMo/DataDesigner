...
<Warning title="Row-count changes">
  Data Designer enforces row-count invariance in `process_before_batch()`. The async engine also enforces it in `process_after_batch()`. Run row-filtering or expansion logic in `process_after_generation()`, which operates on the final dataset and supports row-count changes, or put the transform at a workflow boundary.
</Warning>
