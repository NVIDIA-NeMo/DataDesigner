...
<Note title="Full Schema Available During Generation">
...
For a runnable processor example, open the [Word document generation notebook](https://colab.research.google.com/github/NVIDIA-NeMo/DataDesigner/blob/main/docs/colab_notebooks/8-generating-word-documents.ipynb). It uses the `data-designer-docx` plugin to render one `.docx` file per generated row and store its path in the dataset.
