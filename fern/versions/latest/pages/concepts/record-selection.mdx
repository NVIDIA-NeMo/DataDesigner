...
boolean expression, custom column, or plugin column. Data Designer owns candidate generation, filtering, refill,
deterministic trimming, checkpoints, and resume.

Use record selection when filtering after generation would leave fewer rows than you requested. For example, you
can derive a boolean expression from a judge score, validation result, safety decision, or nested structured model
output before a row enters the output.

## Configure an accepted-row target

Declare the criterion as a boolean expression column, then attach `RecordSelectionConfig` to the builder:
...
The predicate column must exist in the dataset configuration. A direct built-in predicate must be an
`ExpressionColumnConfig` with `dtype="bool"`. Custom and registered plugin columns may be direct predicates; their
values are validated at runtime. Sampler, seed, validation, and built-in model columns are not direct predicates.
Normalize their values through a boolean expression first. For example, extract a nested structured decision with:

```python
builder.add_column(
    dd.ExpressionColumnConfig(
        name="meets_criteria",
        expr="{{ judgment.accepted }}",
        dtype="bool",
        drop=True,
    )
)
...
Here, `judgment` may be an `LLMStructuredColumnConfig`; record selection still reads only the top-level
`meets_criteria` predicate column and does not interpret JSON paths itself.
...
Image-generation columns are not supported with record selection in V1. Data Designer rejects such configurations
before generation and names every unsupported image column in the error. Custom and plugin code should be
side-effect-free: external files that it creates are not tracked by record selection, so artifacts associated with
rejected candidates may remain.
...
Published output is rebuilt from immutable accepted partitions after a crash. Record-selection-specific Hugging Face
publication behavior is outside V1 and will be handled through the repository-wide publishing path separately.
