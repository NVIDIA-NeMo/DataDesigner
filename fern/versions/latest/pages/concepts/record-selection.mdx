...
<Warning>
  ...
  positive integer that reflects your cost limit. Booleans, floats, and numeric strings are rejected instead of
  being coerced into a budget.
</Warning>

## Predicate behavior

The predicate column must exist in the dataset configuration. An expression predicate must use `dtype="bool"`;
plugin columns with unknown output types are validated at runtime.
