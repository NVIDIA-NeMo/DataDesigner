...
`{{ person.openness.description }}`, `{{ person.occupation }}`, `{{ person.county }}` all become available to downstream Jinja templates immediately. See the [Person Sampling docs](/concepts/person-sampling) for the full setup walkthrough (NGC API key + `data-designer download personas --locale en_US`).
