...
**Resolve the `data-designer` command before doing anything else.** Run `which data-designer 2>/dev/null || realpath .venv/bin/data-designer 2>/dev/null`. If the command is not on PATH but exists at `.venv/bin/data-designer`, use the **absolute** path printed by `realpath` for ALL `data-designer` commands throughout this skill. If neither works, see the Troubleshooting section.
