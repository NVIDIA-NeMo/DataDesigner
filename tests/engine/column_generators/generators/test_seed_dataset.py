...
from data_designer.config.seed_dataset import HuggingFaceSeedConfig, LocalFileSeedConfig
...
def stub_seed_dataset_config():
    return SeedDatasetMultiColumnConfig(
        ...
        config=HuggingFaceSeedConfig(dataset="hf://datasets/test/dataset"),
    )
...
def stub_seed_dataset_generator(stub_resource_provider, stub_duckdb_conn, stub_seed_dataset_config):
    ...
    mock_seed_dataset_reader = mock_provider.seed_dataset_reader
    mock_seed_dataset_reader.create_duckdb_connection.return_value = stub_duckdb_conn
    mock_seed_dataset_reader.get_dataset_uri.return_value = "test_uri"
...
def test_seed_dataset_column_generator_metadata():
    ...
    assert ResourceType.SEED_DATASET_READER in metadata.required_resources
...
def test_seed_dataset_column_generator_config_structure():
    ...
    assert config.config.dataset == "hf://datasets/test/dataset"
...
def create_generator_with_real_file(
    ...
    config = SeedDatasetMultiColumnConfig(
        ...
        config=LocalFileSeedConfig(path=file_path),
        # config=LocalFileSeedConfig(path=f"test/{os.path.basename(file_path)}"),
    ...
    mock_provider = stub_resource_provider
    mock_seed_dataset_reader = mock_provider.seed_dataset_reader
    mock_seed_dataset_reader.create_duckdb_connection.return_value = real_conn
    mock_seed_dataset_reader.get_dataset_uri.return_value = file_path
...
def test_seed_dataset_generator_ordered_sampling(fixture_name, stub_resource_provider, request):
    ...
    real_conn = duckdb.connect()
    mock_provider = stub_resource_provider
    mock_provider.seed_dataset_reader.create_duckdb_connection.return_value = real_conn
    mock_provider.seed_dataset_reader.get_dataset_uri.return_value = file_path
