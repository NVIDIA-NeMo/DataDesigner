...
from data_designer.config.seed_dataset import DataFrameSeedConfig, HuggingFaceSeedConfig
...
def test_getters(stub_data_designer_builder):
    ...
    assert stub_data_designer_builder.get_seed_config().config.dataset == "hf://datasets/test-repo/testing/data.csv"
...
def test_seed_config(stub_complete_builder):
    ...
    assert seed_config.config.dataset == "hf://datasets/test-repo/testing/data.csv"
...
def test_with_seed_dataset_basic(stub_empty_builder):
    ...
    dataset = "hf://datasets/test-repo/test-data.parquet"
    result = stub_empty_builder.with_seed_dataset(HuggingFaceSeedConfig(dataset=dataset))

    assert result is stub_empty_builder
    assert stub_empty_builder.get_seed_config().config.dataset == dataset


def test_with_seed_dataset_sampling_strategy(stub_empty_builder):
    """Test with_seed_dataset with different sampling strategies."""
    config = HuggingFaceSeedConfig(dataset="hf://datasets/test-repo/test-data.parquet", token="test-token")
...
def test_to_from_config_with_dataframe_seed(stub_model_configs, caplog):
    builder = DataDesignerConfigBuilder(model_configs=stub_model_configs)

    df = pd.DataFrame(data={"hello": [1, 2], "world": [10, 20]})
    df_seed = DataFrameSeedConfig(df=df)
    ...
    with tempfile.NamedTemporaryFile(suffix=".yaml") as tmp_file:
        builder.write_config(tmp_file.name)
        assert "DataFrame" in caplog.text

        caplog.clear()

        new_builder = DataDesignerConfigBuilder.from_config(tmp_file.name)
        assert "DataFrame" in caplog.text

    assert new_builder.get_seed_config() is None
    cols = new_builder.get_column_configs()
    assert len(cols) == 1
    assert cols[0] == sampler_column
