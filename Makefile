...
ifdef NOFILE
	@PERF_OUTPUT=$$(uv run python -X importtime -c "import data_designer.config; from data_designer.interface import DataDesigner" 2>&1); \
	echo "$$PERF_OUTPUT"; \
	echo ""; \
	echo "Summary:"; \
	...
	uv run python -X importtime -c "import data_designer.config; from data_designer.interface import DataDesigner" > "$$PERF_FILE" 2>&1; \
